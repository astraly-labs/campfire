...
private struct Palette {
    static let paper = NSColor(calibratedRed: 0.965, green: 0.953, blue: 0.929, alpha: 1)
    static let ink = NSColor(calibratedRed: 0.075, green: 0.075, blue: 0.08, alpha: 1)
    static let ember = NSColor(calibratedRed: 47 / 255, green: 92 / 255, blue: 216 / 255, alpha: 1)
    static let blue = NSColor(calibratedRed: 0.28, green: 0.42, blue: 0.92, alpha: 1)
    static let line = NSColor(calibratedWhite: 0.14, alpha: 0.13)
    static let muted = NSColor(calibratedWhite: 0.34, alpha: 1)
}
...
private func makeOriginalScore(output: URL) throws {
    ...
    // Entirely synthesized here: D major/add9 at 90 BPM, no samples or third-party audio.
    let notes: [(start: Double, beats: Double, midi: Double, gain: Double)] = [
        (0.00, 6.0, 38, 0.06), (0.00, 6.0, 45, 0.04),
        (1.33, 0.7, 62, 0.11), (2.00, 0.7, 69, 0.10), (2.67, 1.2, 66, 0.10),
        (4.00, 0.7, 62, 0.11), (4.67, 0.7, 69, 0.10), (5.33, 1.2, 71, 0.10),
        (6.67, 0.7, 66, 0.11), (7.33, 0.7, 69, 0.10), (8.00, 1.4, 74, 0.09),
        (10.00, 0.8, 69, 0.10), (10.67, 0.8, 71, 0.09), (11.33, 1.3, 74, 0.10),
        (13.20, 2.6, 62, 0.08), (13.20, 2.6, 66, 0.06), (13.20, 2.6, 69, 0.05),
    ]
    let beat = 60.0 / 90.0
    ...
    for channel in 0..<2 {
        ...
        for index in 0..<Int(frameCount) {
            ...
            for note in notes {
                ...
                let noteDuration = note.beats * beat
                let local = time - noteStart
                guard local >= 0, local < noteDuration else { continue }
                let attack = min(1, local / 0.045)
                let release = min(1, (noteDuration - local) / 0.5)
                let envelope = attack * release * exp(-local * 0.35)
                let frequency = midiFrequency(note.midi)
                let sine = sin(2 * .pi * frequency * local)
                let upper = sin(2 * .pi * frequency * 2 * local) * 0.18
                let pan = channel == 0 ? 0.98 : 1.02
                value += (sine + upper) * envelope * note.gain * pan
            }
            ...
            let fadeIn = min(1, time / 0.7)
            let fadeOut = min(1, (duration - time) / 0.8)
            samples[index] = Float(tanh(value * fadeIn * fadeOut * 1.8) * 0.72)
        }
    }
