...
const make = Effect.gen(function* () {
  ...
  const start: ProviderCommandReactorShape["start"] = Effect.fn("start")(function* () {
    ...
    const replayPendingTurnStarts = Effect.gen(function* () {
    ...
    const resumePendingWork = clearInterrupted.pipe(Effect.andThen(replayPendingTurnStarts));
    const activation = yield* ServerActivation;
    if (activation === undefined) {
      yield* resumePendingWork;
    } else {
      yield* forkParked(resumePendingWork);
    }
  });
