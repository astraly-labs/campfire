...
import {
  makeLiveSubscriptionBuffer,
  type LiveSubscriptionBufferOptions,
} from "./liveSubscriptionBuffer.ts";
...
interface ThreadLiveEventCoalescerOptions<E> {
  readonly coalesceWindow?: Duration.Input;
  readonly outputBuffer?: LiveSubscriptionBufferOptions<E>;
}
...
export const makeThreadLiveEventCoalescer = Effect.fn("makeThreadLiveEventCoalescer")(function* <
  E = never,
>(options?: ThreadLiveEventCoalescerOptions<E>) {
  const output = options?.outputBuffer
    ? yield* makeLiveSubscriptionBuffer<OrchestrationThreadStreamItem, E>(options.outputBuffer)
    : yield* Effect.gen(function* () {
        const queue = yield* Queue.unbounded<OrchestrationThreadStreamItem>();
        return {
          offer: (item: OrchestrationThreadStreamItem) =>
            Queue.offer(queue, item).pipe(Effect.asVoid),
  ...
  const process = Effect.fn("ThreadLiveEventCoalescer.process")(function* (input: ThreadLiveInput) {
    yield* mutex.withPermits(1)(
      Effect.gen(function* () {
        ...
        if (input.kind === "event") {
          ...
          yield* flushPending();
          yield* output.offer({ kind: "synchronized" });
        }
      }),
    );
  });
