...
const sideThreadId = SideThreadId.make("side-message-anchor");
const actor = {
...
it.layer(NodeServices.layer)("durable orchestration SideThreads", (it) => {
  it.effect("creates, posts, and archives with the authenticated event author", () =>
    Effect.gen(function* () {
  ...
  it.effect("rejects missing anchors and unauthenticated authors", () =>
    Effect.gen(function* () {
      ...
      expect(missingActor.message).toContain("authenticated client actor");
    }),
  );
});
