...
it.layer(Layer.mergeAll(NodeServices.layer, ServerSettingsService.layerTest(), TestHttpClientLive))(
  ...
  (it) => {
    ...
    describe("checkClaudeProviderStatus", () => {
      ...
      it.effect(
        "includes Claude Opus 4.8 with xhigh as the default effort on supported versions",
        () =>
          Effect.gen(function* () {
            const status = yield* checkClaudeProviderStatus(
              defaultClaudeSettings,
            ...
            if (!opus48) {
              assert.fail("Expected Claude Opus 4.8 to be present for Claude Code v2.1.161.");
            }
            if (!opus48.capabilities) {
              assert.fail(
                "Expected Claude Opus 4.8 capabilities to be present for Claude Code v2.1.161.",
              );
            }
      ...
      it.effect("hides Claude Opus 4.8 but keeps 4.7 on versions between the two gates", () =>
        Effect.gen(function* () {
          ...
          assert.strictEqual(
            status.message,
            "Claude Code v2.1.111 is too old for Claude Opus 4.8. Upgrade to v2.1.161 or newer to access it.",
          );
        }).pipe(
          Effect.provide(
            mockSpawnerLayer((args) => {
