...
describe("ProviderSessionReaper", () => {
    ...
    const layer = makeProviderSessionReaperLive({
      ...
      Layer.provideMerge(
        Layer.succeed(ProjectionSnapshotQuery, {
          ...
          getActiveThreadByWorktreePath: () => Effect.die("unused"),
  ...
  it("skips stale sessions when the thread has a pending user-input question", async () => {
    const threadId = ThreadId.make("thread-reaper-pending-input");
    const now = "2026-01-01T00:00:00.000Z";
    const harness = await createHarness({
      readModel: makeReadModel([
        {
